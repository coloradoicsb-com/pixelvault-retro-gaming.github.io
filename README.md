# PIXELVAULT Retro Gaming Store

A premium static e-commerce storefront concept for classic consoles, retro games, controllers, arcade gear, accessories and collectibles.

## Highlights

- 24 product entries across 9 departments
- Responsive desktop/tablet/mobile layout
- Search and category filtering
- Sorting by price and name
- Product quick-view modal
- Cart with quantity controls and localStorage persistence
- Wishlist with localStorage persistence
- Curated collections
- Journal/editorial section
- Dedicated support center
- Support phone and email contact
- Lightweight local image assets
- No external framework or build step required

## Run locally

Open `index.html` in a browser. For the best local-development experience, serve the folder with any simple static server.

## Images

The product catalog imagery in this demo was generated specifically for the project and then optimized into individual JPEG assets for fast loading. The PIXELVAULT logo is also included as a lightweight JPEG asset.

## Support

Demo support contact:
- support@pixelvault.example
- +1 (305) 555-0198

These are placeholder demo contacts and should be replaced before production use.

## Technologies

HTML5 • CSS3 • JavaScript • LocalStorage

## Status

Project in development.

© 2026 PIXELVAULT Retro Gaming
